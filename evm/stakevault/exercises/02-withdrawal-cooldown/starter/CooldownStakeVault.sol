...
/// @title CooldownStakeVault (STARTER)
/// @notice Exercise 02 — add a per-user withdrawal cooldown to the canonical
///         StakeVault. The reward-accumulator machinery below is the verified
///         Chapter 13 code; your job is to fill in the cooldown logic marked
///         with `// TODO`.
/// @dev Acceptance criteria:
///      - each `stake` (re)arms a per-user timer of `cooldownPeriod` seconds,
///      - `withdraw` reverts with `WithdrawalLocked(unlockAt)` before the timer
///        elapses and succeeds at/after it,
///      - `claim` is NOT gated (rewards stay liquid during the lock),
///      - `setCooldownPeriod` lets the owner tune the window.
contract CooldownStakeVault is Ownable, ReentrancyGuard {
    ...
    function stake(uint256 amount) external nonReentrant updateReward(msg.sender) {
        ...
        // TODO: (re)arm this user's cooldown timer from `block.timestamp`.
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        // TODO: revert with WithdrawalLocked(unlockAt[msg.sender]) if the
        //       cooldown has not elapsed for msg.sender.
        totalStaked -= amount;
        balanceOf[msg.sender] -= amount;
    ...
    /// @notice Owner lever for the cooldown length.
    function setCooldownPeriod(uint256 _cooldownPeriod) external onlyOwner {
        // TODO: update `cooldownPeriod` and emit `CooldownUpdated`.
